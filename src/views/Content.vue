<template>
  <div style="position: relative">
    ...
    <Modal></Modal>
    <Carrito></Carrito>
    <b-pagination v-model="currentPage" :total-rows="rows" :per-page="perPage" aria-controls="itemList" align="center"></b-pagination>
  </div>
</template>
    ...
    productosData() {
      ...
      for (let i = 0; i < this.getAllProductos.length; i++) {
        if (this.$route.params.name == undefined) {
          ...
          // Muestra los productos según la busqueda
          this.getAllProductos[i].category.name.toLowerCase().includes(this.$route.params.name.toLowerCase()) ||
          this.getAllProductos[i].name.toLowerCase().includes(this.$route.params.name.toLowerCase())
        ) {
          data.push(this.getAllProductos[i]);
        } else if (
